...
public class PowerUp : MonoBehaviour
{
    ...
    [Tooltip("x holds a min value and y a max value for a Rnadom.Range() call.")]
    ...
    void Update()
    {
        ...
        if (!bndCheck.isOnScreen) {
            // If the PowerUp had drifted entirely off screen, destroy it
            Destroy( gameObject );
        }
    }

    public eWeaponType Type { get { return type; } set { SetType(value); } }

    public void SetType(eWeaponType wt) {
        // Grab the WeaponDefinition from Main
        WeaponDefinition def = Main.GET_WEAPON_DEFINITION(wt);
        cubeMat.color =  def.powerUpColor;  // Set the color of PowerCube
        //letter.color = def.color;         // We coudl colorize the letter too
